# This dataset is created on the fly as a sum of the age-specific population half child variants popMprojHigh and popFprojHigh
...
popprojHigh <- local({
	...
	tpopM <- sum.by.country(popMprojHigh)
	tpopF <- sum.by.country(popFprojHigh)
	...
	cbind(country_code=tpopM$country_code, 
				 name=popMprojHigh[,name.col][match(tpopM$country_code, popMprojHigh$country_code)],
